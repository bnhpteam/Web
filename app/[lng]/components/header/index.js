...
export const Header = () => {
    ...
    const currentLng = LANGS.find(l => pathName.startsWith(`/${l.code}`))?.code || 'en';
    ...
    // Switch language: replace current lang segment in path
    const switchLang = (targetLng) => {
        if (targetLng === currentLng) return;
        // Replace /{currentLng} with /{targetLng} in pathname
        const newPath = pathName.replace(`/${currentLng}`, `/${targetLng}`);
        // Set cookie for persistence
        document.cookie = `i18next=${targetLng}; path=/; max-age=31536000`;
        router.push(newPath);
    };
            ...
            <header className={`w-screen h-[56px] fixed top-0 left-0 z-[999] bg-[#101010] border-b border-[#2B2B2B] max-yt:px-[10px]`}>
                <div className='max-w-[1280px] w-full mx-auto h-full border-l border-r border-[#2B2B2B] flex items-center justify-between px-6'>
                    ...
                    <div className='flex items-center gap-4'>
                        ...
                        <a href='/' className='h-[24px] w-[120px] block flex-shrink-0'>
                            <svg width="120" height="24" viewBox="0 0 120 24" fill="none">
                                ...
                                <path d="M43.3944 21.5754H28.1097V2.15354H42.9797C44.014 2.15354 44.8934 2.51829 45.6129 3.2478C46.3324 3.97731 46.6972 4.85171 46.6972 5.87102V8.44928C46.6972 9.90829 46.0126 11.0225 44.6435 11.777C46.2874 12.4965 47.1119 13.6657 47.1119 15.2747V17.8529C47.1119 18.8722 46.7521 19.7466 46.0276 20.4761C45.3081 21.2056 44.4287 21.5704 43.3894 21.5704L43.3944 21.5754ZM40.7012 9.70343C40.9411 9.70343 41.1409 9.61849 41.2958 9.4536C41.4507 9.28871 41.5306 9.09384 41.5306 8.86899V7.31505C41.5306 7.09519 41.4507 6.90033 41.2958 6.73044C41.1409 6.56056 40.9411 6.48061 40.7012 6.48061H33.2663V9.69843H40.7012V9.70343ZM41.1209 17.2483C41.3408 17.2483 41.5357 17.1634 41.7055 16.9985C41.8704 16.8336 41.9554 16.6387 41.9554 16.4139V14.8599C41.9554 14.6401 41.8704 14.4452 41.7055 14.2753C41.5407 14.1055 41.3458 14.0255 41.1209 14.0255H33.2713V17.2433H41.1209V17.2483Z" fill="white" />
                                <path d="M71.2206 21.5754H66.0591L55.7111 9.50856V21.5754H50.5496V2.15354H55.7111L66.0591 14.2204V2.15354H71.2206V21.5754Z" fill="white" />
                                <path d="M95.549 21.5754H90.3876V14.0305H80.2594V21.5754H75.0979V2.15354H80.2594V9.69844H90.3876V2.15354H95.549V21.5754Z" fill="white" />
                        ...
                        {/* Language switcher */}
                        <div className='flex items-center border border-[#2B2B2B] overflow-hidden' style={{ height: 26 }}>
                            {LANGS.map((lang, idx) => (
                                <button
                                    key={lang.code}
                                    onClick={() => switchLang(lang.code)}
                                    className={`px-3 text-[11px] tracking-widest transition-all duration-200 h-full
                                        ${idx > 0 ? 'border-l border-[#2B2B2B]' : ''}
                                        ${currentLng === lang.code
                                            ? 'bg-[#C6AC6F] text-[#000] font-semibold'
                                            : 'text-[rgba(255,255,255,0.35)] hover:text-white hover:bg-[#1A1A1A]'
                                        }`}
                    ...
                    <div className='flex items-center gap-x-6'>
                        {/* Home link */}
                        <a href='/' className={`whitespace-nowrap text-[14px] text-[rgba(255,255,255,0.4)] ease-out duration-[.2s] cursor-pointer hover:!text-white ${pathName === '/' || pathName === '/en' || pathName === '/zh' ? '!text-white' : ''}`}>
                        ...
                        <div ref={dropdownRef} className='relative'>
                            <button
                                ...
                                className={`flex items-center gap-1.5 whitespace-nowrap text-[14px] ease-out duration-[.2s] cursor-pointer hover:!text-white ${navPages.some(p => pathName.includes(p.href)) ? 'text-white' : 'text-[rgba(255,255,255,0.4)]'}`}
                            >
                                ...
                                <div className='absolute top-[calc(100%+12px)] right-0 bg-[#101010] border border-[#2B2B2B] min-w-[160px] z-[1000] shadow-xl'>
                                    {navPages.map((item, idx) => (
                                        <a
                                            ...
                                            className={`flex items-center justify-between px-4 py-3 text-[13px] border-b border-[#2B2B2B] last:border-b-0 ease-out duration-[.2s] hover:bg-[#1A1A1A] hover:text-white group ${pathName.includes(item.href) ? 'text-white' : 'text-[rgba(255,255,255,0.4)]'}`}
                                        >
                                            <span>/ {item.name}</span>
